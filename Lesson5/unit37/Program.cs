...
int [] numbers = {1, 2, 6, 5, 7, 5};
int [] number2 = new int [(numbers.Length + 1) / 2];

for (int i = 0; i < number2.Length; i++)
{
    number2[i] = numbers[i] * numbers[numbers.Length -1 - i];// длина -1, чтобы не выйти за длину массива
}
if (numbers.Length % 2 != 0)
{
    number2[number2.Length - 1] = numbers[numbers.Length/2];
}
Console.WriteLine($" результат {String.Join(", ", number2)}");